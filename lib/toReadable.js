...
const availableIdentifiers = require('./_identifiers');

const defaultIdentifiers = Object.entries(availableIdentifiers)
  .reduce((defaultIdentifiers, [type, ids]) => {
    defaultIdentifiers[type.toLowerCase()] = ids[ids.length - 1];
...
module.exports = function toReadable(ms, options = {}) {
  ...
  ms = +ms;
  options = Object.assign({ identifiers: defaultIdentifiers, sep: ' ' }, options);
